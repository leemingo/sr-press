...
class _FeatureExtractionLayer(nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self.bn_1 = nn.BatchNorm2d(32)  # BatchNorm for first convolution
        self.bn_2 = nn.BatchNorm2d(64)  # BatchNorm for second convolution

    def forward(self, x):
        #x = F.relu(self.conv_1(x))
        x = F.relu(self.bn_1(self.conv_1(x)))  # Apply BatchNorm after Conv
        x = self.symmetric_padding(x)
        #x = F.relu(self.conv_2(x))
        x = F.relu(self.bn_2(self.conv_2(x)))  # Apply BatchNorm after Conv
        x = self.symmetric_padding(x)
        return x
...
class _PredictionLayer(nn.Module):
    ...
    def __init__(self):
        ...
        self.bn1 = nn.BatchNorm2d(32)  # BatchNorm for first 1x1 convolution

    def forward(self, x: torch.Tensor): 
        #x = F.relu(self.conv1(x))
        x = F.relu(self.bn1(self.conv1(x)))  # Apply BatchNorm after Conv
...
class ToSoccerMapTensor:
    ...
    def __call__(self, sample):
        ...
        nb_prev_actions = 3
        target = int(sample["possession_change_by_4_actions"]) if "possession_change_by_4_actions" in sample else None
...
#         presser_coo = frame.loc[frame.actor, ["x", "y"]].values.reshape(-1, 2)  # Location of the presser
#         ball_coo = np.array([[start_x, start_y]]) # Location of the ball(or event player)
#         goal_coo = np.array([[105, 34]]) # Location of the goal

#         players_att_coo = frame.loc[frame.teammate, ["x", "y"]].values.reshape(-1, 2)  # Locations of the presser's teammates
#         players_def_coo = frame.loc[~frame.teammate, ["x", "y"]].values.reshape(-1, 2) # Locations of the presser's opponents

#         # Output
#         matrix = np.zeros((8, self.y_bins, self.x_bins))

#         # CH 1: Locations of presser: sparse matrix
#         x_bin_press, y_bin_press = self._get_cell_indexes(presser_coo[:, 0], presser_coo[:, 1])
#         matrix[0, y_bin_press, x_bin_press] = 1

#         # CH 2: Locations of presser's teammates
#         x_bin_att, y_bin_att = self._get_cell_indexes(players_att_coo[:, 0], players_att_coo[:, 1])
#         # for i in range(len(x_bin_att)):
#         #     matrix[1, y_bin_att[i], x_bin_att[i]] += 1
#         matrix[1, y_bin_att, x_bin_att] = 1

#         # CH 3: Locations of defending team
#         x_bin_def, y_bin_def = self._get_cell_indexes(players_def_coo[:, 0],players_def_coo[:, 1])
#         # for i in range(len(x_bin_def)):
#         #     matrix[2, y_bin_def[i], x_bin_def[i]] += 1
#         matrix[2, y_bin_def, x_bin_def] = 1

#         # CH 3: Distance to ball(or event player's start_location)
#         yy, xx = np.ogrid[0.5 : self.y_bins, 0.5 : self.x_bins]

#         x0_ball, y0_ball = self._get_cell_indexes(ball_coo[:, 0], ball_coo[:, 1])
#         x0_ball, y0_ball = x0_ball+0.5, y0_ball+0.5 # _get_cell_indexes은 bottom-left의 위치로 설정되기 때문에 grid의 center로 변환
#         matrix[3, :, :] = np.sqrt((xx - x0_ball) ** 2 + (yy - y0_ball) ** 2)

#         # CH 4: Distance to goal
#         x0_goal, y0_goal = self._get_cell_indexes(goal_coo[:, 0], goal_coo[:, 1])
#         x0_goal, y0_goal = x0_goal+0.5, y0_goal+0.5 # _get_cell_indexes은 bottom-left의 위치로 설정되기 때문에 grid의 center로 변환
...
#         b = ball_coo_bin - coords
#         matrix[5, :, :] = np.clip(
#             np.sum(a * b, axis=2) / (np.linalg.norm(a, axis=2) * np.linalg.norm(b, axis=2) + np.finfo(float).eps), -1, 1
#         )
...
#         matrix[7, :, :] = np.abs(
#             np.arctan((y0_goal - coords[:, :, 1]) / ((x0_goal - coords[:, :, 0]) + np.finfo(float).eps))
#         )
